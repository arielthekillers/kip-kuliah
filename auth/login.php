...
document.getElementById('loginForm').addEventListener('submit', function(e) {
  ...
  fetch('<?= BASE_URL ?>/ajax/auth_login.php', {
    method: 'POST',
    body: formData
  })
  ...
  .then(data => {
    if (data.success) {
      alertBox.className = 'mb-6 rounded-xl px-4 py-3 text-sm flex gap-3 items-start border bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300';
      alertMessage.innerHTML = data.message;
      alertBox.classList.remove('hidden');
      window.location.href = data.redirect;
    } else {
      alertBox.className = 'mb-6 rounded-xl px-4 py-3 text-sm flex gap-3 items-start border bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300';
      alertMessage.innerHTML = data.message;
      alertBox.classList.remove('hidden');
